...
"""
    ...
    # Mint each element's id up front so it can be embedded into kpis_json/
    # charts_json AND used as the audit_trail row's key -- the frontend's
    # click-to-inspect looks a dashboard element up by this id.
    kpis = output.get("kpis", [])
    for kpi in kpis:
        kpi["element_id"] = uuid.uuid4().hex
    for chart in resolved_charts:
        chart["element_id"] = uuid.uuid4().hex
